######################################################################
# Name: Humichip_ResRatio_Visit_subcategory1.R
# Author: Ryan Johnson
# Date Created: 24 July 2018
# Purpose: Determine which subcategory1 categories are significantly altered
...
Humichip_RR <- Humichip %>%
  ...
  # Calculate the Response Ratio (RR) for each subcategory1 (V4-V1, V5-V1)
...
Humichip_RR_tidy <- Humichip_RR %>%
  select(subcategory1, RR_41, RR_51, CI95_41, CI95_51) %>%
  gather(key = RR_group, value = RR, -subcategory1, -CI95_41, -CI95_51) %>%
  gather(key = CI_group, value = CI95, -subcategory1, -RR_group, -RR) %>%
