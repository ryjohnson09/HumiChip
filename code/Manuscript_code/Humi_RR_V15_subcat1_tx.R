...
humi_RR <- humi_grouped %>% 
  ...
  # Compress NAs
  ungroup() %>%
  group_by(Gene_category, Subcategory1, Treatment) %>%
...
ggsave(plot = RR_plot, filename = "results/figures/Manuscript_Figures/RR_V15_subcat1_tx.png", 
       height = 6, 
       width = 7)
