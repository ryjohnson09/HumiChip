######################################################################
# Name: Humi_RR_subcat1_ESBLtaq_V1.R
# Author: Ryan Johnson
...
humi_RR <- humi_grouped %>% 
  ...
  group_by(Gene_category, Subcategory1, ESBL_pos_V1) %>% 
  ...
  ungroup() %>%
  group_by(Gene_category, Subcategory1, ESBL_pos_V1) %>%
  spread(ESBL_pos_V1, mean_signal) %>% 
...
ggsave(plot = RR_plot, filename = "results/figures/Manuscript_Figures/RR_V1_subcat1_ESBL_taq.png", 
       height = 9, 
       width = 9)
