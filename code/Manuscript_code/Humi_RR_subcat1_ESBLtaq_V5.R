######################################################################
# Name: Humi_RR_subcat1_ESBLtaq_V5.R
# Author: Ryan Johnson
# Date Created: 5 Aug 2019
# Purpose: Determine which genes categories are significantly altered.
#   Comparing ESBL pos vs negative by taq (any ESBL genes detected)
#   at V1
######################################################################
...
humi_grouped <- humi_grouped %>% 
  filter(!is.na(ESBL_pos_V5))

## Calculate Response Ratio ------------------------------------------------------------------------

humi_RR <- humi_grouped %>% 
  # Group by Subcategory1 and others
  group_by(Gene_category, Subcategory1, ESBL_pos_V5) %>% 
  ...
  ungroup() %>%
  group_by(Gene_category, Subcategory1, ESBL_pos_V5) %>%
  spread(ESBL_pos_V5, mean_signal) %>% 
...
ggsave(plot = RR_plot, filename = "results/figures/Manuscript_Figures/RR_V5_subcat1_ESBL_taq.png", 
       height = 9, 
       width = 9)
