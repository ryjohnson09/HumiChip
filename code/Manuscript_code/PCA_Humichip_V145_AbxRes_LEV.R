####################################################################
# Name: PCA_Humichip_V145_AbxRes_LEV.R
# Author: Ryan Johnson
# Date Created: 4 Sep 2019
# Purpose: Perform PCA on Humichip Data, visit 1-4-5 samples, color
#   by visit. Only consider patients in LEV group. Only use Abx
#   Res probes
####################################################################
...
treat_filter <- treat %>% 
  ...
  # Only include RIF tx group
  filter(Treatment == "LEV")
...
humi_PCA_plot <- humi_PCA_base +
  ...
  stat_ellipse(aes(x = PC1, y = PC2, color = visit_number), level = 0.75, size = 1.5, linetype = "dashed") +
  scale_color_discrete(breaks=c(1, 4, 5),
                       labels=c("Day 0", "Day 7", "Day 21"))

humi_PCA_plot

ggsave(plot = humi_PCA_plot, filename = "results/figures/Manuscript_Figures/Humi_V145_AbxRes_LEV_PCA.png", 
       height = 7, 
       width = 8)
