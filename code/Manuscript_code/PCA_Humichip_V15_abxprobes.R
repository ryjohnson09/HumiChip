####################################################################
# Name: PCA_Humichip_V15_abxprobes.R
# Author: Ryan Johnson
# Date Created: 3 Sep 2019
# Purpose: Perform PCA on Humichip Data, visit 1-5 samples, only
#  consider Abx Resistance probes
####################################################################

library(tidyverse)
library(vegan)

probe_type <- "ANTIBIOTIC_RESISTANCE"
visits <- c(1, 5)
...
humi_PCA_plot <- humi_PCA_base +
  ...
  scale_color_discrete(breaks=c(1, 5),
                       labels=c("Day 0", "Day 21"))

humi_PCA_plot

ggsave(plot = humi_PCA_plot, filename = "results/figures/Manuscript_Figures/Humi_V15_abxprobes_PCA.png", 
       height = 7, 
       width = 8)
