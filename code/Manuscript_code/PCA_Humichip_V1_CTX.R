##################################################################
# Name: PCA_Humichip_V1_CTX.R 
# Author: Ryan Johnson
# Date Created: 4 Sep 2019
# Purpose: Perform PCA on Humichip Data, visit 1, colored
#   CTX pos or neg (based on taq)
##################################################################
...
treat_filter <- treat %>% 
  ...
  # Remove NA in CTX_either_V1
  filter(!is.na(CTX_either_V1))
...
humi_PCA_plot <- humi_PCA_base +
  ...
  stat_ellipse(aes(x = PC1, y = PC2, color = CTX_either_V1), level = 0.75, size = 1.5, linetype = "dashed")

humi_PCA_plot

ggsave(plot = humi_PCA_plot, filename = "results/figures/Manuscript_Figures/Humi_V1_CTX.png", 
       height = 7, 
       width = 8)
