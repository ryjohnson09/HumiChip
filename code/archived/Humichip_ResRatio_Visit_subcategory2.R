##########################################################################
# Name: Humichip_ResRatio_Visit_subcategory2.R
# Author: Ryan Johnson
# Date Created: 24 July 2018
# Purpose: Determine which genes subcategories2 are significantly altered
...
Humichip_RR <- Humichip %>%
  ...
  select(-geneCategory, -Genbank.ID, -gene,  
         -Signal, -species, -subcategory1, -annotation, -lineage) %>%
  ...
  # Calculate the Response Ratio (RR) for each subcategory2 (V4-V1, V5-V1)
...
Humichip_RR_tidy <- Humichip_RR %>%
  select(subcategory2, RR_41, RR_51, CI95_41, CI95_51) %>%
  gather(key = RR_group, value = RR, -subcategory2, -CI95_41, -CI95_51) %>%
  gather(key = CI_group, value = CI95, -subcategory2, -RR_group, -RR) %>%
...
subcategory2_no_overlap_0 <- Humichip_RR_tidy %>%
  mutate(overlap_zero = ifelse(0 > RR - CI95 & 0 < RR + CI95, TRUE, FALSE)) %>%
  filter(overlap_zero == FALSE) %>%
  distinct(subcategory2) %>%
  pull(subcategory2)

Humichip_RR_tidy <- Humichip_RR_tidy %>%
  filter(subcategory2 %in% subcategory2_no_overlap_0)

rm(subcategory2_no_overlap_0)

# Factor for plot
Humichip_RR_tidy$subcategory2 <- factor(Humichip_RR_tidy$subcategory2, 
                             levels = rev(sort(unique(Humichip_RR_tidy$subcategory2))))
...
Humichip_RR_plot <- ggplot(data = Humichip_RR_tidy) +
  ...
  geom_point(aes(x = subcategory2, y = RR, color = RR_group), size = 4, position = dodge) +
  geom_errorbar(aes(ymin = RR - CI95, ymax = RR + CI95, x = subcategory2, color = RR_group), position = dodge) +
  ...
  labs(title = "Response Ratio",
       subtitle = "Subcategory2 by Visit",
       x = "Subcategory2 Name",
       y = "Response Ratio",
...
ggsave("results/figures/Humichip_RespRatio_Visit_subcategory2.png", height = 5, width = 15)
